...
bool QueryEvaluator::evaluateSelectQueryGroups() {
    for (auto& queryGroup : query.getSelectQueryGroups()) {
        synTable.setSynonymStorage(SELECT_STORAGE(queryGroup.getSynonymCounter(), queryGroup.getSelectSynonyms()));
        if (!evaluateQueryGroup(queryGroup)) {
            return false;
        }
        synTable.mergeToResult();
    }
    return true;
}
...
bool QueryEvaluator::evalauteNonSelectQueryGroups() {
    for (auto& queryGroup : query.getNonSelectQueryGroups()) {
        synTable.setSynonymStorage(TEMP_STORAGE(queryGroup.getSynonymCounter()));
        if (!evaluateQueryGroup(queryGroup)) {
            return false;
        }
    }
...
ANSWER QueryEvaluator::evaluateSelectClause(QuerySelectResult& result) {
    synTable.setSynonymStorage(query.getSelectResult().getSelectElementList());
