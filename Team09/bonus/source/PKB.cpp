...
void PKB::setNextBipPopulated(bool populated) {
	pkbCache.nBipCache.setPopulated(populated);
}

void PKB::setNextBipStarPopulated(bool populated) {
	pkbCache.nBipStarCache.setPopulated(populated);
}

void PKB::setAffectsBipPopulated(bool populated) {
	pkbCache.aBipCache.setPopulated(populated);
}

void PKB::setAffectsBipStarPopulated(bool populated) {
	pkbCache.aBipStarCache.setPopulated(populated);
}
...
INDICES& PKB::getIndexedDEValuesOf(DESIGN_ENTITY deType) {
	...
	case DESIGN_ENTITY::ASSIGN: case DESIGN_ENTITY::CALL: case DESIGN_ENTITY::WHILE: case DESIGN_ENTITY::IF:
...
StatementStorage& PKB::getStmtStore() {
	return stmtStore;
}
...
bool PKB::hasRelationship(RELATIONSHIP_TYPE rType, VAR firstValue, VAR secondValue) {
	...
	if (!firstValue.empty()) {
		return (firstIndex && !secondValue.empty()) ? relStore.hasRelationship(firstIndex, secondIndex)
...
VALUES PKB::getFirstValuesOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY firstValueType, VAR secondValue, VALUES& tableValues) {
	...
	if (!secondValue.empty()) {
		...
		for (auto& firstValue : tableValues) {
			if (isOfDEType(firstValueType, firstValue) && relStore.hasRelationship(firstValue, secondIndex)) {
		...
		for (auto& firstIndex : tableValues) {
			if (isOfDEType(firstValueType, firstIndex) && relStore.hasRelationshipWithFirstValue(firstIndex)) {
...
VALUES PKB::getSecondValuesOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY secondValueType, VAR firstValue, VALUES& tableValues) {
	if (tableValues.empty()) {
	...
	if (!firstValue.empty()) {
		...
		for (auto& secondValue : tableValues) {
			if (isOfDEType(secondValueType, secondValue) && relStore.hasRelationship(firstIndex, secondValue)) {
		...
		for (auto& secondIndex : tableValues) {
			if (isOfDEType(secondValueType, secondIndex) && relStore.hasRelationshipWithSecondValue(secondIndex)) {
...
VALUE_PAIRS PKB::getValuePairsOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY firstValueType, DESIGN_ENTITY secondValueType, VALUE_PAIRS& tableValuePairs) {
...
VALUE_PAIRS PKB::getValuePairsOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY firstValueType, DESIGN_ENTITY secondValueType, VALUES& tableValues, bool isFirstValues) {
	if (tableValues.empty()) {
		return getValuePairsOf(rType, firstValueType, secondValueType);
	}
...
VALUE_PAIRS PKB::getValuePairsOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY firstValueType, DESIGN_ENTITY secondValueType) {
	auto& relStore = getRelationshipStoreOf(rType);
	VALUE_PAIRS valuePairs;
...
VALUES PKB::getRelationshipToSelfOf(RELATIONSHIP_TYPE rType, DESIGN_ENTITY valueType, VALUES& tableValues) {
	if (tableValues.empty()) {
...
void PKB::populateNextBipCache() {
	pkbCache.nBipCache.populateCache();
}
void PKB::populateNextBipStarCache() {
	pkbCache.nBipStarCache.populateCache();

}
void PKB::populateAffectsBipCache() {
	pkbCache.aBipCache.populateCache();

}
void PKB::populateAffectsBipStarCache() {
	pkbCache.aBipStarCache.populateCache();
}
...
VALUES PKB::getAssignOfPattern(VAR_NAME varName, PATTERN pattern, bool isExactMatch, VALUES& tableValues) {
	if (tableValues.empty()) {
...
VALUE_PAIRS PKB::getAssignVarOfPattern(PATTERN pattern, bool isExactMatch, VALUES& tableValues, bool isAssignValues) {
	if (tableValues.empty()) {
		return getAssignVarOfPattern(pattern, isExactMatch);
	}
...
VALUE_PAIRS PKB::getValuePairsOf(DESIGN_ENTITY firstValueType, DESIGN_ENTITY secondValueType, VALUE_PAIRS& tableValuePairs) {
	if (tableValuePairs.empty()) {
...
VALUE_PAIRS PKB::getValuePairsOf(DESIGN_ENTITY firstValueType, DESIGN_ENTITY secondValueType, VALUES& tableValues) {
	if (tableValues.empty()) {
		return getValuePairsOf(firstValueType, secondValueType);
	}
...
VALUE_PAIRS PKB::getStmtVarPairsOf(DESIGN_ENTITY firstStmtType, DESIGN_ENTITY secondStmtType, VALUE_PAIRS& tableValuePairs) {
	if (tableValuePairs.empty()) {
	...
	for (auto& stmtVarPair : tableValuePairs) {
		if (stmtStore.getIndexedVarOf(firstStmtType, stmtVarPair.first) == stmtStore.getIndexedVarOf(secondStmtType, stmtVarPair.second)
...
VALUE_PAIRS PKB::getStmtVarPairsOf(DESIGN_ENTITY firstStmtType, DESIGN_ENTITY secondStmtType, VALUES& tableValues, bool isFirstValues) {
	if (tableValues.empty()) {
		return getStmtVarPairsOf(firstStmtType, secondStmtType);
	}
...
VALUE_PAIRS PKB::getStmtVarVarPairsOf(DESIGN_ENTITY stmtType, DESIGN_ENTITY varType, VALUE_PAIRS& tableValuePairs) {
	if (tableValuePairs.empty()) {
	...
	VALUE_PAIRS stmtVarPairs;
	for (auto& stmtVarPair : tableValuePairs) {
		if (stmtStore.getIndexedVarOf(stmtType, stmtVarPair.first) == stmtVarPair.second && isOfDEType(varType, stmtVarPair.second)) {
			stmtVarPairs.insert(stmtVarPair);
		}
	}
	return stmtVarPairs;
...
VALUE_PAIRS PKB::getStmtVarVarPairsOf(DESIGN_ENTITY stmtType, DESIGN_ENTITY varType, VALUES& tableValues, bool isStmtValues) {
	if (tableValues.empty()) {
		return getStmtVarVarPairsOf(stmtType, varType);
	}
...
void PKB::clearCache() {
	...
	pkbCache.nBipCache.clear();
	pkbCache.nBipStarCache.clear();
	pkbCache.aBipCache.clear();
	pkbCache.aBipStarCache.clear();
}
